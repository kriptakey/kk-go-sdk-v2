...
-----END CERTIFICATE REQUEST-----`
	...
	csrResponse, err := connection.KK_SignCertificate(1, session.SessionToken, DEMO_KEY_ID, 365, "SHA256", DEMO_CSR)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	fmt.Println("- KK_SignCertificate: ", protojson.Format(csrResponse))

	verifyResponse, err := connection.KK_VerifyCertificate(1, session.SessionToken, DEMO_KEY_ID, csrResponse.Certificate)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	fmt.Println("- KK_VerifyCertificate: ", protojson.Format(verifyResponse))
