package main

import (
	"fmt"
	"log"
	"os"

	"google.golang.org/protobuf/encoding/protojson"

	kk "github.com/kriptakey/kk-go-sdk-v24.1/kriptakey"
)

func main() {

	// Change these constants to the actual value in your environment
	DEMO_HOSTNAME := "target-kk-cs.com"
	DEMO_PORT := 7005

	DEMO_PARTITION_ID := 1
	DEMO_PARTITION_PASSWORD := "Password1!"

	DEMO_CLIENT_CERTIFICATE := "/PathToClient/Cert.pem"
	DEMO_CLIENT_PRIVATE_KEY := "/PathToClientKey/Priv.key"

	DEMO_WRAPPING_KEY_ID := "AESWrappingKey"
	DEMO_WRAPPING_KEY_ID_RSA := "RSAWrappingKey"
	DEMO_PUBLIC_KEY := `-----BEGIN PUBLIC KEY-----
	
	 -----END PUBLIC KEY-----`

	connection, err := kk.KK_InitializeConnection(DEMO_HOSTNAME, uint16(DEMO_PORT), DEMO_CLIENT_CERTIFICATE, DEMO_CLIENT_PRIVATE_KEY)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}

	session, err := connection.KK_AppAuthenticate(uint32(DEMO_PARTITION_ID), DEMO_PARTITION_PASSWORD)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	fmt.Println("- Session: ", protojson.Format(session))

	var keyLength uint32 = 4096
	response, err := connection.KK_GenerateAppstoredKeypair(1, session.SessionToken, "internalKey", DEMO_WRAPPING_KEY_ID, "", "RSA", &keyLength, false)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	fmt.Println("- KK_GenerateAppstoredKeypair: ", protojson.Format(response))

	response2, err := connection.KK_GenerateAppstoredKeypair(1, session.SessionToken, "internalKey", DEMO_WRAPPING_KEY_ID_RSA, "", "RSA", &keyLength, true)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	fmt.Println("- KK_GenerateAppstoredKeypair: ", protojson.Format(response2))

	// Please apply license to run the below sample
	response3, err := connection.KK_GenerateAppstoredKeypair(1, session.SessionToken, "appstoredPublicKey", "", DEMO_PUBLIC_KEY, "RSA", &keyLength, true)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	fmt.Println("- KK_GenerateAppstoredKeypair: ", protojson.Format(response3))
}
