...
func main() {
	...
	fileEncryptResponse, err := connection.FileEncrypt(1, session.SessionToken, DEMO_KEY_ID, filePath, filePathOutput)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	err = connection.FileDecryptWithIntegrity(1, session.SessionToken, DEMO_KEY_ID, fileEncryptResponse.KeyVersion, fileEncryptResponse.Iv, fileEncryptResponse.Tag, filePathOutput, plaintextPath)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
