...
func main() {
	...
	response, err := connection.GenerateExternalKeypair(1, session.SessionToken, "internalKey", DEMO_WRAPPING_KEY_ID, "RSA", &keyLength, true)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	fmt.Println("- GenerateExternalKeypair: ", protojson.Format(response))

	response2, err := connection.GenerateExternalKeypair(1, session.SessionToken, "internalKey", DEMO_PUBLIC_KEY, "RSA", &keyLength, true)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	fmt.Println("- GenerateExternalKeypair: ", protojson.Format(response2))
}
