...
func main() {
	...
	response, err := connection.GenerateExternalKey(1, session.SessionToken, "internalKey", DEMO_WRAPPING_KEY_ID, DEMO_PUBLIC_KEY, 32)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	fmt.Println("- GenerateExternalKey: ", protojson.Format(response))
}
