...
func main() {
	...
	DEMO_PUBLIC_KEY_OR_CERT := `-----BEGIN PUBLIC KEY-----
	...
	ciphertext, err := kk.KK_SealForTransit([]byte(data), DEMO_PUBLIC_KEY_OR_CERT)
	if err != nil {
		log.Fatal(err)
	}

	plaintext, err := connection.KK_UnsealDataFromTransit(1, session.SessionToken, DEMO_WRAPPING_KEY_ID, DEMO_WRAPPED_KEY, ciphertext)
	if err != nil {
		log.Fatal(err)
	}
