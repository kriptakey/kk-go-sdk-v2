...
func main() {
	...
	encryptRequest.Plaintext = append(encryptRequest.Plaintext,
		&kkreq.APIRequest_SingleEncrypt{
			Plaintext: "Klavis",
	...
	for i, _ := range encrypted.Ciphertext {
		decryptRequest.Ciphertext = append(decryptRequest.Ciphertext,
			&kkreq.APIRequest_SingleDecrypt{
				Ciphertext: encrypted.Ciphertext[i].Ciphertext,
				Iv:         encrypted.Ciphertext[i].Iv,
				Mac:        encrypted.Ciphertext[i].Mac,
				Aad:        wrapperspb.String("aad1"),
				KeyID:      DEMO_KEY_ID,
