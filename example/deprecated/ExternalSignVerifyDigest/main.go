...
func main() {
	...
	signResponse, err := connection.ExternalSignDigest(1, session.SessionToken, DEMO_WRAPPING_KEY_ID, DEMO_WRAPPED_KEY, "sha256Hashed", "tmaJPUUYhYKxPwzK+qbxE2n2b6qNYqUecihlMemwP4I=")
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	...
	verify, err := connection.ExternalVerifyDigest(1, session.SessionToken, DEMO_PUBLIC_KEY, "sha256Hashed", "tmaJPUUYhYKxPwzK+qbxE2n2b6qNYqUecihlMemwP4I=", signResponse.Signature)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
