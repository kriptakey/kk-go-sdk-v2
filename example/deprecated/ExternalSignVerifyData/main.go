...
func main() {
	...
	signResponse, err := connection.ExternalSignData(1, session.SessionToken, DEMO_WRAPPING_KEY_ID, DEMO_WRAPPED_KEY, "SHA256", "datatobesigned")
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	...
	verify, err := connection.ExternalVerifyData(1, session.SessionToken, DEMO_PUBLIC_KEY, "SHA256", "datatobesigned", signResponse.Signature)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
