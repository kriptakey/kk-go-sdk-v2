...
func main() {
	...
	signResponse, err := connection.SignData(1, session.SessionToken, DEMO_KEY_ID, "SHA256", "RSASSA-PKCS1-v1_5", "datatobesigned")
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	...
	verify, err := connection.VerifyData(1, session.SessionToken, DEMO_KEY_ID, "SHA256", "RSASSA-PKCS1-v1_5", "datatobesigned", signResponse.Signature)
	fmt.Println(verify)
}
