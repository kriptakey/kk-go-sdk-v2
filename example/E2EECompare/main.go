...
func main() {
	...
	var e2eeCompareWith kkreq.APIRequestE2EECompare_Comparewith
	e2eeCompareWith.PermanentKeyId = wrapperspb.String(DEMO_PERMANENT_KEY_ID)
	e2eeCompareWith.Algo = wrapperspb.String(DEMO_SESSION_KEY_ALGO)
	e2eeCompareWith.KeyVersion = wrapperspb.UInt32(0)

	var ciphertexts []*kkreq.APIRequestE2EECiphertext
	singleCiphertext1 := kkreq.APIRequestE2EECiphertext{Text: wrapperspb.String("Nk0T/MM="), Mac: wrapperspb.String("8ttVYJPyvU36LUiLdqbdjA=="), Iv: wrapperspb.String("BVpi1HjiwaRNolZO")}
	ciphertexts = append(ciphertexts, &singleCiphertext1)
	singleCiphertext2 := kkreq.APIRequestE2EECiphertext{Text: wrapperspb.String("dJajVSs="), Mac: wrapperspb.String("dMSuRgZXyxBwOmziTLJmSA=="), Iv: wrapperspb.String("GeUd/aVvq/FqwKRd")}
	ciphertexts = append(ciphertexts, &singleCiphertext2)

	e2eeCompareWith.Ciphertext = ciphertexts

	e2eeCompareResponse, err := connection.KK_E2EECompare(uint32(DEMO_PARTITION_ID), session.SessionToken, &e2eeSourceRequest, &e2eeCompareWith)
	if err != nil {
		log.Fatal(err.Error())
		os.Exit(1)
	}
	fmt.Printf("- E2EECompare: %s", protojson.Format(e2eeCompareResponse))
}
